def parse_forms(form, checkboxes: list[str] = []):
    ...
    checkboxes are a list of input:[type=checkbox]`s property_names. If you want checkbox in resulted dict only for
    specific id, specify it: chekboxes = ["specific_id:checkbox_name1", "specific_id:checkbox_name2"]
    ...
    for id_, res_dict in result.items():
        for checkbox in checkboxes:
            specific_id = None
            if ":" in checkbox:
                specific_id, checkbox = checkbox.split(":")
            if checkbox not in res_dict:
                if not specific_id == id_:
                    continue
